...
+(void)createAnimationLayerWithString:(NSString*)title name:(NSString*)name andRect:(CGRect)rect andView:(UIView *)view andFont:(UIFont*)ui_font andStrokeColor:(UIColor*)color
{
    YQAnimationLayer * animationLayer = [YQAnimationLayer layer];
    animationLayer.frame = rect;
    [view.layer addSublayer:animationLayer];
   
    NSString*string = [title stringByAppendingString:name];
    ...
    CFArrayRef runArray = CTLineGetGlyphRuns(line);
     CGSize titleSize = [title sizeWithAttributes:@{ NSFontAttributeName : [UIFont fontWithName:ui_font.fontName size:ui_font.pointSize] }];
    // for each RUN
    for (CFIndex runIndex = 0; runIndex < CFArrayGetCount(runArray); runIndex++)
    {
        ...
        for (CFIndex runGlyphIndex = 0; runGlyphIndex < CTRunGetGlyphCount(run); runGlyphIndex++)
        {
            ...
            if(runGlyphIndex >=title.length){
               
                if(title.length%2==0){
                    position.x = titleSize.width/2-titleSize.width/(title.length*2);
                    
                }else{
                    position.x = titleSize.width/2;
                }
                position.y = -((runGlyphIndex-title.length+1)*ui_font.pointSize);
            }
            {
...
//    [path appendPath:[UIBezierPath bezierPathWithCGPath:letters]];
    [path appendPath:[UIBezierPath bezierPathWithCGPath:letters]];
//    [path moveToPoint:CGPointMake(rect.size.width
//                                  /2, 0)];
//
//    [path addLineToPoint:CGPointMake(rect.size.width
//                                     /2, 60)];
    CGPathRelease(letters);
    CFRelease(font);
    
    CAShapeLayer *pathLayer = [CAShapeLayer layer];
    pathLayer.frame = CGRectMake(0, 0, rect.size.width, rect.size.height-ui_font.pointSize*name.length );
